...
instance (Datatype d, GToSchema f) => GToSchema (D1 d f) where
    gToSchema =
      ...
      where
        ...
        addDatatypeName mName dName (Builder x (Fix (Prod Nothing cs))) =
            Builder x . Fix . Prod (Just (mName, dName)) $ cs
...
instance (Constructor c, GToSchema f) => GToSchema (C1 c f) where
    gToSchema = M1 <$> addConName (conName (undefined :: C1 c f a)) gToSchema
      where
        ...
        addConName name (Builder x (Fix Empty)) = Builder x . Fix . Atom . TyName $ name
        addConName name (Builder x s)           = Builder x . Fix . Con name $ s
